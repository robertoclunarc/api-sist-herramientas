import  DetalleEntrada  from 'api/models/detallesentradas.models';
...
export async function getDetalleEntradaPorId(id: number): Promise<DetalleEntrada | null> {
    ...
    const result = await db.querySelect('SELECT * FROM detallesentradas WHERE iddetalleentrada = ?', [id]);
...
export async function createDetalleEntrada(detalleentrada: DetalleEntrada): Promise<DetalleEntrada>{
    ...
    const result = await db.querySelect('INSERT INTO detallesentradas SET ?',[detalleentrada]);
...
export async function updateDetalleEntrada(detalleentrada: DetalleEntrada): Promise<boolean> {
    ...
    const result = await db.querySelect('UPDATE detallesentradas SET ? WHERE idetalleentrada',[detalleentrada, detalleentrada.iddetalleentrada]);
...
export async function deleteDetalleEntrada(id: number): Promise<boolean> {
    ...
    const result = await db.querySelect('DELETE FROM detalleentrada WHERE iddetalleentrada = ?', [id]);
    return result[0].affectedRows > 0;
